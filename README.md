...
Trades list rows and Details show Gross P&L with its currency for closed Trades when it can be calculated. Net P&L stays `—` when costs are unknown, with an explicit commission/fees explanation; a calculated zero is displayed as `0`. Trades list row color follows known Net P&L; when Net is unavailable, a known Gross loss receives loss styling, while a Gross gain stays neutral because unknown costs may change the Net outcome. Average entry and exit prices are shown with up to eight fractional digits in the list and Details; full stored and calculation precision is unchanged. The imported MNQ sample with two contracts, a 1.25-point gain, and a 2 USD point value has 5 USD Gross P&L and unknown Net P&L.
