...
Execution timestamps are entered explicitly as `America/New_York` wall-clock time and accept only `yyyy-MM-dd HH:mm:ss` or `yyyy-MM-dd HH:mm`. A trailing `Z`, an explicit offset, and culture-specific dates are rejected. Desktop converts each value through the shared trading-time policy before constructing the Application command; nonexistent spring-forward times and ambiguous fall-back times are rejected with specific validation. The resulting command and Domain market facts remain zero-offset UTC and never depend on the Windows local timezone.
...
Opened (New York), Trade, Account, Average Prices, Open Qty, and Net P&L are clickable sort headers; Actions is not sortable. Only the active header shows an up/down arrow. New columns default to ascending except the canonical `OpenedAtUtc` sort and Net P&L, which default to descending; clicking the active column toggles direction and every sort returns to page 1. Average Prices sorts by exact average entry price, Open Qty by exact open quantity, and Net P&L by exact numeric outcome with open/null Trades always last. Every order ends with Trade ID ascending for stable page boundaries.
